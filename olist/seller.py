...
class Seller:
    ...
    def get_active_dates(self):
        ...
        # Compute dates
        orders_sellers["date_first_sale"] = orders_sellers["order_approved_at"].copy()
        orders_sellers["date_last_sale"] = orders_sellers["order_approved_at"].copy()
    ...
    def get_review_score(self):
        ...
        """
        # Get order-level review data and map it to sellers via order_items
        ...
        # Aggregate review metrics per seller
        ...
        # Estimate the cost to Olist of each review score
        # (bad reviews are assumed to cost Olist customer trust / retention)
        ...
        df['cost'] = df['review_score'].apply(review_cost)
        cost = df.groupby('seller_id', as_index=False).agg(
            {'cost': 'sum'})
        cost.columns = ['seller_id', 'cost_of_reviews']

        return res.merge(cost, on='seller_id')
    ...
    def get_training_data(self):
        ...
        # Olist revenue model: 10% commission on sales + 80 BRL/month subscription fee
        training_set['revenues'] = (
            training_set['sales'] * 0.10
            + 80 * training_set['months_on_olist']
        )

        # Profit = revenues minus the cost of bad reviews
